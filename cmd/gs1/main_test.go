...
func TestParseStrictCheckDigit(t *testing.T) {
	if code, _, _ := exec(t, "", "parse", "-strict", "0104150000021126"); code != 0 {
		t.Errorf("valid strict parse exit %d, want 0", code)
	}
	code, _, errOut := exec(t, "", "parse", "-strict", "0104150000021127")
	if code != 1 || !strings.Contains(errOut, "invalid check digit") {
		t.Errorf("invalid strict parse exit %d stderr %q", code, errOut)
	}
}
