...
func runParse(args []string, stdin io.Reader, stdout, stderr io.Writer) int {
	...
	fs.BoolVar(&opts.strict, "strict", false, "validate GTIN check digits for AI 01 and 02")
...
func parseOne(input string, b *gs1.Barcode, stdout io.Writer, opts parseOptions) error {
	parseOpts := gs1.ParseOptions{ValidateCheckDigits: opts.strict}
	if err := gs1.ParseIntoWithOptions(input, b, parseOpts); err != nil {
		return err
	}
